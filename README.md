...
The code for calibrating camera is in calibrate_camera function which requires following input arguments: globpattern,nx,ny,image_shape
The function reads in all calibration images one by one, convert to gray scale and using cv2.findChessboardCorners to search for inner corners (intersection of two blaclk squares) and append them in imgpoints list. A corresponding object point is then appended to objpoints list. imgpoints and objpoints are then used to calibrate the camera via cv2.calibrateCamera. Function returns cameraMatrix(mtx),  distortion coefficients(dist), rotation vectors(rvecs), and translation vectors(tvecs).
...
I used combination of SobelX gradient and Saturation(S) channel (HLS space) thresholding to create thresholded binary image. For S thresholding,  I set ( s_thresh_min, s_thresh_max) to (170,255).
...
By trying several combinations of four source and destination points  and checking which combination will produce a straight and vertical lane lines for the perspective transformation of the  straight_lines1.jpg and straight_lines2.jpg, source and destinations points obtained were:
...
The code sniffet below is wrapped inside sliding_window function the purpose of which is to find the left and right lane-line pixels and perform a second order polynomial fit on pixels positions to obtain the left_fit and right_fit polynomial coefficients. 
...
###Pipeline (video)
Here's a [link to my video result](./project4_AnnotatedVideo.mp4)
