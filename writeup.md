...
I used combination of SobelX gradient, Saturation(S) channel (HLS space) and Red color (RGB) thresholding to create thresholded binary image (P4AdvancedLaneLines.py line 316). For S thresholding,  I set ( s_thresh_min, s_thresh_max) to (100,255) and for Red color thresholding (min,max) are (200,255).
The resulting s_binary image is:
![alt text][image3]
---
The resulting r_binary image is:
![alt text][image12]
---
Finding the common pixels between Saturation and Red image masked  out the pixels due to shadow and sudden road color change.

Resulting image of ANDing Saturation and Red image:
![alt text][image13]
----
...
![alt text][image4]
---
Combining the image obtained from ANDing Saturation and Red color binary with sobelx binary will give the final binary image:

```
 combined = np.zeros_like(s_binary)
 combined[((s_binary == 1) & (r_binary == 1))  | (gradx == 1)] = 1
showPlotBeforeAfter(undist,combined,"undistorted test image","Combination of (S and R) and  SobelX Gradient", cmap='gray')
```
![alt text][image5]
---
